...
def verify():
    try:
        ...
        for anci in anchor_image_tensors:
            ...
            if dist <= 0.125139:  # Threshold obtained using Test.py
                return jsonify({"match": True, "error": False, "threshold":"%.6f" % (0.125139), "distance":"%.6f"%(mindist)})
        return jsonify({"match": False, "error": False, "threshold":0.125139, "distance":round(mindist, 6)})
    except Exception as e:
        print(e)
        return jsonify({"error":True})
